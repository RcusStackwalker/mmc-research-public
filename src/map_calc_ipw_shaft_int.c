void calc_ipw_shaft_int(uint16_t p0, uint16_t p1)
{
	...
	unsigned l_samples = s_add(map_samples, map_samples_prev);
	if (l_samples) {
		map_avg16 = s16_divu32_16(add16(map_samples_sum, map_samples_sum_prev) * 256, l_samples);
		map_avg8 = conv16to8(map_avg16);
	}
	...
	if (run_state_extended_flags & 0x02) {
		adc_byte_map_delayed1 = adc_byte_map;
		adc_byte_map_delayed2 = adc_byte_map_delayed1;
		adc_byte_map_delayed3 = adc_byte_map_delayed2;
		adc_byte_map_delayed4 = adc_byte_map_delayed3;

		uint16_t l_map = tps_based_failsafe_map8;
		map_avg8 = l_map;
		map_avg16 = l_map * 256;
	}

	if (const_atmospheric_pressure <= baro_compensated_load8) {
		load_source_values_compensated_sharped = map_avg16;
	} else if (ram67e4_u16) {
		load_source_values_compensated_sharped = map_avg16;
	} else if (flash2ab2_u16 > ABS_DIFF(adc_byte_map_delayed1, adc_byte_map)) {
		load_source_value_compensated_sharped = map_avg16;
	} else {
		if (adc_byte_map_delayed1 <= adc_byte_map) {
			r2 = s_add16(map_avg16, s_mul_divu16((adc_byte_map - adc_byte_map_delayed1) * 256, flash2542_u16, 32));
		} else {
			r2 = s_sub16(map_avg16, s_mul_divu16((adc_byte_map_delayed1 - adc_byte_map) * 256, flash2544_u16, 32));
		}
		load_source_value_compensated_sharped = mid16(r2, adc_byte_baro_pressure * 256, flash254a_u16);
	}
	if (adc_byte_map >= adc_byte_map_delayed4) {
		max_load_source_value_delta_positive = MAX16(max_load_source_value_delta_positive, adc_byte_map - adc_byte_map_delayed4);
	} else {
		max_load_source_value_delta_negative = MAX16(max_load_source_value_delta_negative, adc_byte_map_delayed4 - adc_byte_map);
	}

	adc_byte_map_stored = adc_byte_map;
	adc_byte_map_delayed4_stored = adc_byte_map_delayed4;

	unsigned l_flag = 1;
	if (ipw_addition_rate) {
		if (flash1672_u16 < ipw_addition_rate) {
			ipw_addition_rate = MAX16(flash1672_u16, s_scale_base256(ipw_addition_rate, ram6c0a_u16));
		} else if (!ram6c1c_u16) {
	...
	|| (run_state_extended_flags & 0x38)) {
		r10 = 0;//
		//if smth - reset fuel film line
		//else
		get_ipw_fuel_film_compensated(0);
	} else if ((run_state-extended_flags & 0x28)
	&& !(run_state_extended_flags & 0x1000)) {
		//reset fuel film line
	} else if ((run_state_extended_flags & 0x01)
	|| (fault_recheck_request_flags & 0x10)
	|| (run_state_extended_flags & 0x03)) {
		r10 = cranking_or_load_sensor_fault_ipw_int;
		//load all film with values
	} else {
		//lsvc
		l_nom_ipw_trimmed = s_mul_divu16(nominal_load_ipw_int, conv8f8_uint(oxygen_feedback_trim) * 2 + o2_offset_trim_int + long_term_fuel_trim_int, 512);
		ret = s_mul_divu16(l_nom_ipw_trimmed, load_source_values_compensated_sharped, 2048);
		ret = s_mul32_16_divu16(ret, 128, aftercranking_enrichment * 2 + 128);
		ret = get_ipw_fuel_film_compensated(s_mul32_16_divu16(ret, 256, purge_fp11570_u16 + 128));
		ret = s32_scale_base128(ret, aftercranking_enrichment * 2 + 128);
		r10 = s32_scale_base256(ret, purge_fp11570_u16 + 128);
		if (ipw_addition)
			r10 = s_add16(r10, ipw_addition);
		if (ipw_subtraction) {
			r10 = s_sub16(r10, ipw_subtraction);
			r10 = MAX16(r10, 1);
		}
	}
	if (MD_ACTIVE) {
	...
	if (run_state_extended_flags & 0x2000) {
		r9 = 2;
		r10 = start_pulse_ipw_int;
		S_DEC(winter_start_decay)
		if (!winter_start_decay)
			run_state_extended_flags &= ~0x2000;
	} else if (run_state_extended_flags & 0x01) {
		r9 = 0;
	} else if (run_state_extended_flags & 0x8000) {
		r9 = 2;
	} else if () {
		r9 = 2;
		r10 = s_divu16(r10, 4);
