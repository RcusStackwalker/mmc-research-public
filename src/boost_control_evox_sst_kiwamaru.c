...
DECLARE_AXIS_WITH_DESC(flash_boost_control_sst_gear_axis,
	SST_GEAR_AXIS_SIZE,
	"Gear",
	"uint16") = {
	TEMP_AXIS_SRC_YDST,
...
#define DECLARE_BOOST_TARGET_LOAD_MAP(symbol, username)				\
	DECLARE_3DMAP8_WITH_DESC(symbol,					\
		...
		SST_GEAR_AXIS_SIZE,						\
		"SST Rcus Moded Boost",						\
		username,							\
		"Load8",							\
		"DDflash_boost_control_sst_gear_axis",				\
...
DECLARE_BOOST_TARGET_LOAD_MAP(flash_boost_target_load_normal_3dmap8, "Target Load - Normal");
DECLARE_BOOST_TARGET_LOAD_MAP(flash_boost_target_load_sport_3dmap8, "Target Load - Sport");
DECLARE_BOOST_TARGET_LOAD_MAP(flash_boost_target_load_supersport_3dmap8, "Target Load - S-Sport");
...
DECLARE_WGDC_MAP(flash_base_wgdc_normal_3dmap8, "Base Wastegate Duty - Normal");
DECLARE_WGDC_MAP(flash_base_wgdc_sport_3dmap8, "Base Wastegate Duty - Sport");
DECLARE_WGDC_MAP(flash_base_wgdc_supersport_3dmap8, "Base Wastegate Duty - S-Sport");
...
static const void *const flash_boost_target_loads[3] = {
	...
	&flash_boost_target_load_supersport_3dmap8
};
...
void new_update_boost_target_load()
{
	...
	calc_axis(&flash_boost_control_sst_gear_axis);

	l_load_base = sst_multimap8u16(flash_boost_target_loads);
...
static const void *const flash_base_wgdcs[3] = {
	&flash_base_wgdc_normal_3dmap8,
	&flash_base_wgdc_sport_3dmap8,
	&flash_base_wgdc_supersport_3dmap8
};
...
uint16_t new_calc_wastegate_reactive_duty()
{
	...
	l_wgdc_base = sst_multimap8u16(flash_base_wgdcs);
